...
The Software has no licence key and no activation, so it contacts no backend of
ours.
...
> tool's own privacy policy. The Software is free software with no licence key and
> no activation, so it sends **nothing** to any backend of ours. The only personal
> data we may process is what is needed to keep the product reliable through
...
This Policy covers personal data processed by the Software itself. It does
**not** cover third-party AI tools,
...
### 3.1 Diagnostic and telemetry data (limited)
...
### 3.2 Data stored only on your device
...
Diagnostic data is kept only as long as necessary for the purposes in
Section 3.1.
...
art. 46). No method of transmission or storage is fully secure, and we cannot
