unit Aefos.OTA.Chat.UI.AboutForm;

{
  Branded About dialog for the Chat host BPL.

  Replaces the plain MessageDlg About with a runtime-built modal form that
  shows the black-background Aefos lockup logo (RCDATA 'AEFOS_ABOUT_LOGO',
  embedded via Chat.UI.Branding.res — already linked by Chat.UI.Branding) on a
  matching black canvas, with the suite/support facts below it.

  All surfaced support facts come from Chat.Core.SupportInfo (ADR-276, single
  source of truth) — no version literal or requirement string is re-declared
  here. The Terminal host BPL has its own equivalent unit (it cannot reach this
  unit across the BPL boundary), kept visually identical.
...
const
  ...
  ABOUT_CAPTION  = 'About Aefos AI (Chat)';
