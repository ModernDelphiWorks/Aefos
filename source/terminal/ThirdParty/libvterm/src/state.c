...
static void state_putglyph_fn(VTermState *state, const uint32_t chars[], int width, VTermPos pos)
{
  VTermGlyphInfo info = {
    .chars = chars,
    .width = width,
...
static void state_erase_fn(VTermState *state, VTermRect rect, int selective)
{
...
static int on_text(const char bytes[], size_t len, void *user)
{
  ...
  if(vterm_unicode_is_combining(codepoints[i])) {
    ...
    if(state->pos.row == state->combine_pos.row && state->pos.col == state->combine_pos.col + state->combine_width) {
      ...
      /* Now render it */
      state_putglyph_fn(state, state->combine_chars, state->combine_width, state->combine_pos);
    }
    else {
  ...
  for(; i < npoints; i++) {
    ...
    state_putglyph_fn(state, chars, width, state->pos);
      ...
      case '8': // DECALN
      {
        ...
        for(pos.row = 0; pos.row < state->rows; pos.row++)
          for(pos.col = 0; pos.col < ROWWIDTH(state, pos.row); pos.col++)
            state_putglyph_fn(state, E, 1, pos);
        break;
      }
...
#define INTERMED(i,b) ((i << 16) | b)
  ...
  case LEADER('?', 0x4a): // DECSED - Selective Erase in Display
    ...
    case 0:
      ...
      if(rect.end_col > rect.start_col)
        state_erase_fn(state, rect, selective);
  ...
  case 0x62: { // REP - ECMA-48 8.3.103
    ...
    while (state->pos.col < col) {
      state_putglyph_fn(state, state->combine_chars, state->combine_width, state->pos);
